    ...
    def update(self, test_acc, optim):
        ...
        if len(self.test_acc_list) == self.window and abs(sum(self.acc_swing)) < self.acc_swing_thresh and abs(abs(sum(self.acc_swing))-sum(list(map(abs, self.acc_swing)))) >self.acc_swing_thresh:
            for op in optim.param_groups:
                op['lr'] /= 5
                if op['lr'] < 5e-4:
                    self.acc_swing_thresh = 0.001 ## 0.5%
                self.test_acc_list = []
                self.acc_swing = []
        ...
        if test_loss < test_loss_max:
            ...
            if args.share_weights:
                ...
                torch.save({'train_loss': train_loss,
                            ...
                            'test_acc': test_acc,
                            'epoch': epoch, 
                            'lr': optim.state_dict()['param_groups'][0]['lr']}, './weights/{}/{}/info.pth'.format(args.dataset, args.model_name)) #lr_scheduler._schedulers[-1].get_last_lr()
                ...
                torch.save({'train_loss': train_loss,
                            ...
                            'test_acc': test_acc,
                            'epoch': epoch, 
                            'lr': optim.state_dict()['param_groups'][0]['lr']}, './weights/{}/{}/info.pth'.format(args.dataset, args.model_name)) #lr_scheduler.get_last_lr()


        if args.share_weights:
            print("Epoch: {}/{}, model: {}, lr: {}".format(epoch, args.n_epochs, args.model_name, optim.state_dict()['param_groups'][0]['lr'])) #  lr_scheduler._schedulers[0].get_last_lr()[0], lr_scheduler._schedulers[-1].get_last_lr()[0]
        else:
            print("Epoch: {}/{}, model: {}, lr: {}".format(epoch, args.n_epochs, args.model_name, optim.state_dict()['param_groups'][0]['lr']))    #lr_scheduler.get_last_lr()[0]     
