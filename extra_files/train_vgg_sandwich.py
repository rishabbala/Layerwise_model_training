...
class MakeVGG(nn.Module):
    ...
    def __init__(self, model_name, num_linear_layers, output_size, dataset, other_size=None):
        ...
        super().__init__()
        self.vgg = VGGBasic(num_layers=other_size, num_linear_layers=num_linear_layers, output_size=output_size, dataset=dataset)
    ...
    for key, values in model.named_parameters():
        if key in child_sd.keys() and key not in optim_params0_keys:
            ...
            if 'downsample' in key:
                ...
                print("Here")
                values.data = torch.ones(values.shape).to(device)
...
if __name__ == "__main__":
    ...
    # Input parameters
    parser.add_argument('--model_name', default='sandwich_vgg11')
    ...
    for epoch in range (args.n_epochs):
        ...
        if scheduler.change:
            ...
            if 'vgg11' in args.model_name:
                if pos == 0:
                    other_size = [1, 1, 1, 1, 1]
                elif pos == 1:
                    other_size = [1, 1, 2, 2, 2]
                else:
            ...
            model = MakeVGG(args.model_name+'_'+str(pos), args.num_linear_layers, output_size, args.dataset, other_size=other_size)
